...
export function classifyCacheability(request: NormalizedRequest): Cacheability {
  ...
  if (/(private\s+key|api\s+key|secret\s+token|access\s+token|bearer\s+token|password|secret|credential|medical|legal|financial|payment|wire|security)/.test(text)) return "never_cache";
